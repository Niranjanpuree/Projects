...
import { IBedTypeChangeRequestReview }                               from "./bedType.model";
...
@Component({
    ...
    providers: [BedTypeService, LikeStagingService, UserService, ChangeService],
})

export class BedTypeReviewComponent implements OnInit {

    usersForAssignment: IUser[] = [];
    assignedReviewer: IAssignReviewer = { assignedToUserId: "-1", changeRequestIds: [] };
    bedTypeChangeRequest: IBedTypeChangeRequestReview;
    ...
    constructor(private bedTypeService: BedTypeService, private likeStagingService: LikeStagingService,
    ...
    ngOnInit() {
        ...
        this.bedTypeService.getChangeRequestStaging(id).subscribe(
            result => {
                this.bedTypeChangeRequest = result;

                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.bedTypeChangeRequest.stagingItem.changeRequestId;
                //this.getLikes(id);
                ...
                if (this.bedTypeChangeRequest.stagingItem.status === "Approved" || this.bedTypeChangeRequest.stagingItem.status === "Deleted" || this.bedTypeChangeRequest.stagingItem.status === "Rejected") {
                    this.reviewCompleted = true;
                } else if (this.bedTypeChangeRequest.stagingItem.status === "PreliminaryApproved") {
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.bedTypeService.submitChangeRequestReview(this.bedTypeChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name} change request ID ${this.bedTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name}  change request ID ${this.bedTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name}  change request ID ${this.bedTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name}  change request ID ${this.bedTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.bedTypeChangeRequest.stagingItem.changeType} ${this.bedTypeChangeRequest.stagingItem.entityName} : ${this.bedTypeChangeRequest.entityStaging.name} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
