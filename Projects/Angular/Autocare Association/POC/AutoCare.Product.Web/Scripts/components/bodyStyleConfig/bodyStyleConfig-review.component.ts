...
import { IBodyStyleConfigChangeRequestReview }                            from "./bodyStyleConfig.model";
...
@Component({
    ...
    providers: [BodyStyleConfigService, BodyTypeService, BodyNumDoorsService, LikeStagingService, UserService, ChangeService],
})

export class BodyStyleConfigReviewComponent implements OnInit {
    bodyStyleConfigChangeRequest: IBodyStyleConfigChangeRequestReview;
    ...
    constructor(private bodyStyleConfigService: BodyStyleConfigService, private bodyTypeService: BodyTypeService, private bodyNumDoorsService: BodyNumDoorsService,
    ...
    ngOnInit() {
        ...
        this.bodyStyleConfigService.getChangeRequestStaging(id).subscribe(
            result => {
                this.bodyStyleConfigChangeRequest = result;
                if (this.bodyStyleConfigChangeRequest.stagingItem.status === "PreliminaryApproved") {
                    this.showPrelimRadio = false;
                }
               this.changeRequestReview.changeRequestId = this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId;

                //payload
                this.bodyTypeService.getBodyTypeDetail(this.bodyStyleConfigChangeRequest.entityStaging.bodyTypeId).subscribe(
                    result => {
                        this.bodyStyleConfigChangeRequest.entityStaging.bodyTypeName = result.name;
                        this.changeRequestReview.changeRequestId = this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId;

                        this.bodyNumDoorsService.getBodyNumDoorsDetail(this.bodyStyleConfigChangeRequest.entityStaging.bodyNumDoorsId).subscribe(
                            result => {
                                this.bodyStyleConfigChangeRequest.entityStaging.numDoors = result.numDoors;
                ...
                //exisitng payLoad
                if (this.bodyStyleConfigChangeRequest.entityCurrent !== null) {
                    this.bodyNumDoorsService.getBodyNumDoorsDetail(this.bodyStyleConfigChangeRequest.entityCurrent.bodyNumDoorsId).subscribe(
                        result => {
                            this.bodyStyleConfigChangeRequest.entityCurrent.numDoors = result.numDoors;

                            this.bodyTypeService.getBodyTypeDetail(this.bodyStyleConfigChangeRequest.entityCurrent.bodyTypeId).subscribe(
                                result => {
                                    this.bodyStyleConfigChangeRequest.entityCurrent.bodyTypeName = result.name;
                                }, error => {
                                    this.toastr.warning(<any>error, ConstantsWarehouse.errorTitle);
                                    this.showLoadingGif = false;
                                });
    ...
    onSubmitReview(deletebuttonclicked: String) {
       ...
       this.changeRequestReview.reviewComment = this.reviewComment;
        this.showLoadingGif = true;
        let bodyStyleConfigIdentity: string = this.bodyStyleConfigChangeRequest.entityStaging.numDoors + ", "  +
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.bodyStyleConfigService.submitChangeRequestReview(this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity} change request ID ${this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity}  change request ID ${this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity} change request ID ${this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity} change request ID ${this.bodyStyleConfigChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
    ...
    private onAssignReviewer() {
        ...
        let bodyStyleConfigIdentity: string =  this.bodyStyleConfigChangeRequest.entityStaging.bodyTypeName;

        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    title: `Your review was assigned to ${user.user}`,
                    body: `Your review for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.bodyStyleConfigChangeRequest.stagingItem.changeType} ${this.bodyStyleConfigChangeRequest.stagingItem.entityName} : ${bodyStyleConfigIdentity} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
