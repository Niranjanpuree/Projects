...
import { IBodyTypeChangeRequestReview }                              from "./bodyType.model";
...
@Component({
    ...
    providers: [BodyTypeService, LikeStagingService, UserService, ChangeService],
})

export class BodyTypeReviewComponent implements OnInit {
    usersForAssignment: IUser[] = [];
    assignedReviewer: IAssignReviewer = { assignedToUserId: "-1", changeRequestIds: [] };
    bodyTypeChangeRequest: IBodyTypeChangeRequestReview;
    ...
    constructor(private bodyTypeService: BodyTypeService, private likeStagingService: LikeStagingService,
    ...
    ngOnInit() {
        ...
        this.bodyTypeService.getChangeRequestStaging(id).subscribe(
            result => {
                this.bodyTypeChangeRequest = result;

                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.bodyTypeChangeRequest.stagingItem.changeRequestId;
                //this.getLikes(id);
                this.likeStagingService.getLikeDetails(id).subscribe(result => {
                 ...
                 if (this.bodyTypeChangeRequest.stagingItem.status === "PreliminaryApproved") {
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.bodyTypeChangeRequest.stagingItem.status];
            }
        }
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.bodyTypeService.submitChangeRequestReview(this.bodyTypeChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name} change request ID ${this.bodyTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name}  change request ID ${this.bodyTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name}  change request ID ${this.bodyTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name}  change request ID ${this.bodyTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.bodyTypeChangeRequest.stagingItem.changeType} ${this.bodyTypeChangeRequest.stagingItem.entityName} : ${this.bodyTypeChangeRequest.entityStaging.name} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
