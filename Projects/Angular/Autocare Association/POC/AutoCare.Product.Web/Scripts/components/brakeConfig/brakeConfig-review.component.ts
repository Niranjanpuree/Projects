...
import { IBrakeConfigChangeRequestReview }                            from "./brakeConfig.model";
...
@Component({
    ...
    providers: [BrakeConfigService, BrakeTypeService, BrakeSystemService, BrakeABSService, LikeStagingService, UserService, ChangeService],
})

export class BrakeConfigReviewComponent implements OnInit {
    brakeConfigChangeRequest: IBrakeConfigChangeRequestReview;
    ...
    constructor(private brakeConfigService: BrakeConfigService, private brakeTypeService: BrakeTypeService, private brakeSystemService: BrakeSystemService,
        private brakeABSService: BrakeABSService, private likeStagingService: LikeStagingService,
    ...
    ngOnInit() {
        ...
        this.brakeConfigService.getChangeRequestStaging(id).subscribe(
            result => {
                this.brakeConfigChangeRequest = result;
                ...
                if (this.brakeConfigChangeRequest.stagingItem.status == "PreliminaryApproved") {
                    this.showPrelimRadio = false;
                }

                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.brakeConfigChangeRequest.stagingItem.changeRequestId;

                //payload
                this.brakeTypeService.getBrakeTypeDetail(this.brakeConfigChangeRequest.entityStaging.frontBrakeTypeId).subscribe(
                    result => {
                        this.brakeConfigChangeRequest.entityStaging.frontBrakeTypeName = result.name;
                        this.changeRequestReview.changeRequestId = this.brakeConfigChangeRequest.stagingItem.changeRequestId;

                        this.brakeTypeService.getBrakeTypeDetail(this.brakeConfigChangeRequest.entityStaging.rearBrakeTypeId).subscribe(
                            result => {
                                this.brakeConfigChangeRequest.entityStaging.rearBrakeTypeName = result.name;

                                this.brakeSystemService.getBrakeSystemDetail(this.brakeConfigChangeRequest.entityStaging.brakeSystemId).subscribe(
                                    result => {
                                        this.brakeConfigChangeRequest.entityStaging.brakeSystemName = result.name;

                                        this.brakeABSService.getBrakeABSDetail(this.brakeConfigChangeRequest.entityStaging.brakeABSId).subscribe(
                                            result => {
                                                this.brakeConfigChangeRequest.entityStaging.brakeABSName = result.name;
                ...
                //exisitng payLoad
                if (this.brakeConfigChangeRequest.entityCurrent !== null) {
                    this.brakeTypeService.getBrakeTypeDetail(this.brakeConfigChangeRequest.entityCurrent.frontBrakeTypeId).subscribe(
                        result => {
                            this.brakeConfigChangeRequest.entityCurrent.frontBrakeTypeName = result.name;

                            this.brakeTypeService.getBrakeTypeDetail(this.brakeConfigChangeRequest.entityCurrent.rearBrakeTypeId).subscribe(
                                result => {
                                    this.brakeConfigChangeRequest.entityCurrent.rearBrakeTypeName = result.name;

                                    this.brakeSystemService.getBrakeSystemDetail(this.brakeConfigChangeRequest.entityCurrent.brakeSystemId).subscribe(
                                        result => {
                                            this.brakeConfigChangeRequest.entityCurrent.brakeSystemName = result.name;

                                            this.brakeABSService.getBrakeABSDetail(this.brakeConfigChangeRequest.entityCurrent.brakeABSId).subscribe(
                                                result => {
                                                    this.brakeConfigChangeRequest.entityCurrent.brakeABSName = result.name;
                                                }, error => {
                                                    this.toastr.warning(<any>error, ConstantsWarehouse.errorTitle);
                                                    this.showLoadingGif = false;
                                                });
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.brakeConfigChangeRequest.stagingItem.status];
            }
        }
        ...
        this.showLoadingGif = true;
        let brakeConfigIdentity: string = this.brakeConfigChangeRequest.entityStaging.frontBrakeTypeName + ", " + this.brakeConfigChangeRequest.entityStaging.rearBrakeTypeName + ", " +
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.brakeConfigService.submitChangeRequestReview(this.brakeConfigChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity} change request ID ${this.brakeConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity}  change request ID ${this.brakeConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity} change request ID ${this.brakeConfigChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity} change request ID ${this.brakeConfigChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.brakeConfigChangeRequest.stagingItem.changeType} ${this.brakeConfigChangeRequest.stagingItem.entityName} : ${brakeConfigIdentity} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
