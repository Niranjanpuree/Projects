...
import { IDriveTypeChangeRequestReview }                           from "./driveType.model";
...
@Component({
    ...
    providers: [DriveTypeService, LikeStagingService, UserService, ChangeService],
})

export class DriveTypeReviewComponent implements OnInit {
    driveTypeChangeRequest: IDriveTypeChangeRequestReview;
    ...
    ngOnInit() {
        ...
        this.driveTypeService.getChangeRequestStaging(id).subscribe(
            result => {

                this.driveTypeChangeRequest = result;

                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.driveTypeChangeRequest.stagingItem.changeRequestId;
                ...
                if (this.driveTypeChangeRequest.stagingItem.status == "PreliminaryApproved") {
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.driveTypeChangeRequest.stagingItem.status];
            }
        }
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.driveTypeService.submitChangeRequestReview(this.driveTypeChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name}  change request ID ${this.driveTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name}  change request ID ${this.driveTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name}  change request ID ${this.driveTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name}  change request ID ${this.driveTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.driveTypeChangeRequest.stagingItem.changeType} ${this.driveTypeChangeRequest.stagingItem.entityName} : ${this.driveTypeChangeRequest.entityStaging.name} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
    ...
    //    // Load existing base vechile with reference from RouteParams
    //    const id = Number(this.route.snapshot.params["id"]);
    //    this.mfrBodyCodeService.getChangeRequestStaging(id).subscribe(
    //        result => {
    //            this.mfrBodyCodeChangeRequest = result;
    //            // setup authorization components
    //            if (this.mfrBodyCodeChangeRequest.stagingItem.status === "PreliminaryApproved") {
    //                this.showPrelimRadio = false;
    //            }

    //            // set review's changerequestid
    //            this.changeRequestReview.changeRequestId = this.mfrBodyCodeChangeRequest.stagingItem.changeRequestId;

    //            //payload

    //            this.mfrBodyCodeChangeRequest.entityStaging.name = result.entityStaging.name;
