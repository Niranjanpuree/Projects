...
@Component({
    ...
    providers: [SubModelService, LikeStagingService, UserService, ChangeService],
})

export class SubModelReviewComponent implements OnInit {
    subModelChangeRequest: ISubModelChangeRequestStagingReview;
    ...
    isReviewCompleted: Boolean = false;
    ...
    constructor(private subModelService: SubModelService, private likeStagingService: LikeStagingService,
    ...
    ngOnInit() {
        ...
        this.subModelService.getChangeRequestStaging(id).subscribe(
            result => {
                this.subModelChangeRequest = result;

                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.subModelChangeRequest.stagingItem.changeRequestId;
                //this.getLikes(id);
                ...
                if (this.subModelChangeRequest.stagingItem.status == "PreliminaryApproved") {
                    this.showPrelimRadio = false;
                }
            },
            error => {
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.subModelChangeRequest.stagingItem.status];
            }
        }
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.subModelService.submitChangeRequestReview(this.subModelChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} change request ID ' ${this.subModelChangeRequest.stagingItem.changeRequestId}' was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} change request ID ' ${this.subModelChangeRequest.stagingItem.changeRequestId}' was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} change request ID ' ${this.subModelChangeRequest.stagingItem.changeRequestId}' failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} change request ID ' ${this.subModelChangeRequest.stagingItem.changeRequestId}' failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.subModelChangeRequest.stagingItem.changeType} ${this.subModelChangeRequest.stagingItem.entityName} : ${this.subModelChangeRequest.entityStaging.name} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
