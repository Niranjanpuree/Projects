...
import { IVehicleToBodyStyleConfigChangeRequestStagingReview }            from "./vehicleToBodyStyleConfig.model";
...
@Component({
    ...
    providers: [VehicleToBodyStyleConfigService, VehicleService, BodyStyleConfigService, LikeStagingService, ChangeService, UserService],
})

export class VehicleToBodyStyleConfigReviewComponent implements OnInit {
    disabled: boolean = false;
    vehicleToBodyStyleConfigChangeRequest: IVehicleToBodyStyleConfigChangeRequestStagingReview;
    ...
    constructor(private vehicleToBodyStyleConfigService: VehicleToBodyStyleConfigService, private likeStagingService: LikeStagingService,
        private toastr: ToastsManager, private route: ActivatedRoute, private router: Router, private vehicleService: VehicleService,
        private bodyStyleConfigService: BodyStyleConfigService, private sharedService: SharedService,
    ...
    ngOnInit() {
        ...
        this.vehicleToBodyStyleConfigService.getChangeRequestStaging(id).subscribe(
            result => {
              this.vehicleToBodyStyleConfigChangeRequest = result;
                if (this.vehicleToBodyStyleConfigChangeRequest.stagingItem.status == "PreliminaryApproved") {
                    this.showPrelimRadio = false;
                  }
                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeRequestId;

                //this.getLikes(id);
                this.likeStagingService.getLikeDetails(id).subscribe(result => {
                    this.likeStagingGet = result;

                    this.vehicleService.getVehicle(this.vehicleToBodyStyleConfigChangeRequest.entityStaging.vehicleId).subscribe(result => {
                        this.vehicleToBodyStyleConfigChangeRequest.entityStaging.vehicle = result;

                        this.bodyStyleConfigService.getBodyStyleConfig(this.vehicleToBodyStyleConfigChangeRequest.entityStaging.bodyStyleConfigId).subscribe(result => {
                            this.vehicleToBodyStyleConfigChangeRequest.entityStaging.bodyStyleConfig = result;
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.vehicleToBodyStyleConfigChangeRequest.stagingItem.status];
            }
        }
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.vehicleToBodyStyleConfigService.submitChangeRequestReview(this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeType} ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.entityName} : ${this.vehicleToBodyStyleConfigChangeRequest.entityStaging.id} change request ID ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeType} ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.entityName} : ${this.vehicleToBodyStyleConfigChangeRequest.entityStaging.id} change request ID ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeType} ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.entityName} : ${this.vehicleToBodyStyleConfigChangeRequest.entityStaging.id} change request ID ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeType} ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.entityName} : ${this.vehicleToBodyStyleConfigChangeRequest.entityStaging.id} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.changeType} ${this.vehicleToBodyStyleConfigChangeRequest.stagingItem.entityName} : ${this.vehicleToBodyStyleConfigChangeRequest.entityStaging.id} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
