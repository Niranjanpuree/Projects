...
import { IVehicleToDriveTypeChangeRequestStagingReview }            from "./vehicleToDriveType.model";
...
@Component({
    ...
    providers: [VehicleToDriveTypeService, VehicleService, DriveTypeService, LikeStagingService, ChangeService, UserService],
})

export class VehicleToDriveTypeReviewComponent implements OnInit {
    disabled: boolean = false;
    vehicleToDriveTypeChangeRequest: IVehicleToDriveTypeChangeRequestStagingReview;
    ...
    constructor(private vehicleToDriveTypeService: VehicleToDriveTypeService, private likeStagingService: LikeStagingService,
        private toastr: ToastsManager, private route: ActivatedRoute, private router: Router, private vehicleService: VehicleService,
        private driveTypeService: DriveTypeService, private sharedService: SharedService,
    ...
    ngOnInit() {
        ...
        this.vehicleToDriveTypeService.getChangeRequestStaging(id).subscribe(
            result => {
                this.vehicleToDriveTypeChangeRequest = result;
                
                if (this.vehicleToDriveTypeChangeRequest.stagingItem.status == "PreliminaryApproved") {
                    this.showPrelimRadio = false;
                }
                // set review's changerequestid
                this.changeRequestReview.changeRequestId = this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId;

                //this.getLikes(id);
                this.likeStagingService.getLikeDetails(id).subscribe(result => {
                    this.likeStagingGet = result;

                    this.vehicleService.getVehicle(this.vehicleToDriveTypeChangeRequest.entityStaging.vehicleId).subscribe(result => {
                        this.vehicleToDriveTypeChangeRequest.entityStaging.vehicle = result;

                        this.driveTypeService.getDriveType(this.vehicleToDriveTypeChangeRequest.entityStaging.driveTypeId).subscribe(result => {
                            this.vehicleToDriveTypeChangeRequest.entityStaging.driveType = result;
    ...
    onSubmitReview(deletebuttonclicked: String) {
        ...
        if (deletebuttonclicked == 'delete') {
            ...
            if (this.isPreliminaryApproveChecked) {
                ...
                this.changeRequestReview.reviewStatus = ChangeRequestStatus[this.vehicleToDriveTypeChangeRequest.stagingItem.status];
            }
        }
        ...
        this.newAcFileUploader.uploadAttachments().subscribe(uploadedFiles => {
            ...
            this.vehicleToDriveTypeService.submitChangeRequestReview(this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId, this.changeRequestReview).subscribe(
                result => {
                    if (result) {
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} change request ID ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteSuccessTitle,
                                body: `Your delete request for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} change request ID ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId} was submitted successfully.`
                            };
                        }
                        ...
                        if (this.changeRequestReview.reviewStatus != ChangeRequestStatus.Deleted) {
                            toasterMessage = {
                                ...
                                body: `Your review for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} change request ID ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        } else {
                            toasterMessage = {
                                title: ConstantsWarehouse.reviewDeleteErrorTitle,
                                body: `Your delete request for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} change request ID ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeRequestId} failed.`
                            };
                        }
        ...
        this.changeService.assignReviewer(this.assignedReviewer).subscribe(result => {
            if (result) {
                let toasterMessage = {
                    ...
                    body: `Your review for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} assigned successfully.`
                }
                this.toastr.success(toasterMessage.body, toasterMessage.title);
                ...
                let toasterMessage = {
                    ...
                    body: `Your assignment for ${this.vehicleToDriveTypeChangeRequest.stagingItem.changeType} ${this.vehicleToDriveTypeChangeRequest.stagingItem.entityName} : ${this.vehicleToDriveTypeChangeRequest.entityStaging.id} failed.`
                }
                this.toastr.warning(toasterMessage.body, toasterMessage.title);
            }
        },
