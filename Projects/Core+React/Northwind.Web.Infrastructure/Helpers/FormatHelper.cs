...
namespace Northwind.Web.Infrastructure.Helpers
{
    public static class FormatHelper
    {
        ...
        public static string AuditLogResourceFormat(string[] resourceTitle, string[] resourceValues)
        {
            ...
            for (int i = 0; i < resourceValues.Length; i++)
            {
                if (i == 0)
                {
                    result.Append("<b>" + resourceValues[i] + "</b>");
                }
                else
                {
                    result.Append("<b>" + resourceTitle[i] + " </b> : " + resourceValues[i]);
                }

                if (resourceValues.Length - 1 != i)  // not last array index..
                    result.Append("</br>");
