...
export class KendoDialog1 extends React.Component<IDialogProps, IDialogState> {
        ...
        if (this.props.buttons.length == 1) {
            if (this.props.buttons[0].length > 0) {
                this.props.buttons[0].forEach((e: any, index: number) => {
                    buttons.push(<Button key={this.props.uniqueKey + "button" + index} primary={e.primary} itemID={index + ""} onClick={this.onButtonClick}>{e.text}</Button>)
                })
            }
            else {
    ...
    openForm() {
        ...
        if (!(!this.props && !this.props.getUrl && this.props.getUrl === "")) {
            ...
            if (prop.getMethod.toLowerCase() === "get") {
                ...
                Remote.postDataText(this.props.getUrl, prop.postData, (data: any) => {
    ...
    render() {
        ...
        return (<div>
            {this.state.visible &&
                <Dialog title={this.props.dialogTitle} width={this.props.dialogWidth} height={this.props.dialogHeight}>
