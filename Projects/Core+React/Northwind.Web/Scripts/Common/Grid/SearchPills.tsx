        ...
        return (<div className="search-pills-container">
            {
                this.props.conditions.map((condition, index) => {
                    if (typeof (condition.Value) === "string") {
                        ...
                        let value2 = "";
                        if (condition.Value2) {
                            value2 = " And '" + numberWithCommas(condition.Value2 + "") + "'";
                        }
                        return <div key={"pill-" + index} className="badge badge-pill badge-secondary">{condition.Attribute.AttributeTitle} {condition.Operator.OperatorTitle} '{numberWithCommas(condition.Value + "")}' {value2} <a href="#" itemID={index.toString()} className="pill-close" onClick={this.onPillDelete}><i className="material-icons">close</i></a></div>
                    }
