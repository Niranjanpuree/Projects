...
export class Pagination extends React.Component<IPaginationProps, IPaginationState> {
    ...
    render() {
        let pageNumbers = [];
        const a = Math.ceil(this.props.totalCount / this.props.pageSize);
        ...
        return (
            <div className="k-pager-wrap k-grid-pager k-widget">
                <a href="javascript:void(0);" className={this.props.activePage === 1 ? 'k-link k-pager-nav k-pager-first k-state-disabled' : 'k-link k-pager-nav k-pager-first'} onClick={() => this.props.onPageChange(1, 0)} title="Go to the first page"><span className="k-icon k-i-seek-w" aria-label="Go to the first page"></span></a>
                ...
                <a href="#" className={this.props.activePage === totalPages ? 'k-link k-pager-nav k-pager-last k-state-disabled' : 'k-link k-pager-nav k-pager-last'} onClick={() => this.props.onPageChange(this.props.activePage + 1, (this.props.activePage * this.props.pageSize))} title="Go to the next page"><span className="k-icon k-i-arrow-e" aria-label="Go to the next page"></span></a>

                <a href="#" className={this.props.activePage === totalPages ? 'k-link k-pager-nav k-pager-last k-state-disabled' : 'k-link k-pager-nav k-pager-last'} onClick={() => this.props.onPageChange(Math.ceil(this.props.totalCount / this.props.pageSize), ((Math.ceil(this.props.totalCount / this.props.pageSize)) * this.props.pageSize) - this.props.pageSize)} title="Go to the last page"><span className="k-icon k-i-seek-e" aria-label="Go to the last page"></span></a>
