...
export class AdvancedSearchDialog extends React.Component<IAdvancedSearchDialogProps, IAdvancedSearchDialogState> {
    ...
    onApply(event: any) {
        ...
        if (this.props.onApply) {
            let conditions: any[] = this.state.selectedConditions;
            conditions.map((v: any) => {
                if (v.Attribute.AttributeType === 3) {
                    v.Value += ""
                    v.Value2 += ""
                    if (!isNaN(v.Value.replace(/\,/g, ''))) {
                        v.Value = parseFloat(v.Value.replace(/\,/g, ''))
                    }
                    if (!isNaN(v.Value2.replace(/\,/g, ''))) {
                        v.Value2 = parseFloat(v.Value2.replace(/\,/g, ''))
                    }
                }
            })
