...
export class AdvancedSearchRow extends React.Component<IAdvancedSearchRowProps, IAdvancedSearchRowState> {
    ...
    componentDidMount() {       
        ...
        if (selectedAttribute && selectedAttribute.AttributeType && (selectedAttribute.AttributeType === 2 || selectedAttribute.AttributeType === 3)) {
            if (selectedOperator && selectedOperator.OperatorType && (selectedOperator.OperatorName === 21 || selectedOperator.OperatorName === 29)) {
                selectedValue = numberWithCommas(this.props.selectedValue + "");
                selectedValue2 = numberWithCommas(this.props.selectedValue2 + "");
            }
            else {
                selectedValue = numberWithCommas(this.props.selectedValue + "");
            }            
    ...
    filterChange = (event: any) => {
        ...
        this.timeout = setTimeout(() => {
            this.setState({
                attributeOptions: this.filterData(event.filter),
                loading: false
            });
        }, this.delay);
    ...
    filterData(filter: any) {
        const data = this.state.attributeAllOptions.slice();
    ...
    renderValue() {
        ...
        switch (attributeType) {
            ...
            case 4:
                ...
                break;
            case 5:
                inputControl = <ComboBox className="form-control" data={this.state.attributeOptions} textField="name" dataItemKey="value" filterable={true} loading={this.state.loading} onFilterChange={this.filterChange} value={this.state.selectedValue} onChange={this.onValueChange}/>;
                break;
            case 6:
                if (operatorType == 33) {
                    let selectedValue = [];
                ...
                else if (operatorType == 32 || operatorType == 40) {
                    let selectedValue = {};
                    if (typeof (this.state.selectedValue) === "object" && this.state.selectedValue && !this.state.selectedValue.length) {
                        selectedValue = this.state.selectedValue
                    }
                    ...
                    inputControl = <ComboBox className="form-control" data={this.state.attributeOptions} textField="name" dataItemKey="value" filterable={true} loading={this.state.loading} onFilterChange={this.filterChange} value={selectedValue} onChange={this.onValueChange}/>;
                }
                break;
            case 7:
                if (operatorType == 35) {
        ...
        switch (attributeType) {
            ...
            case 3:
                if (operatorType == 21 || operatorType == 29) {
                    ...
                    inputControl = <Input required={true} className="form-control" validationMessage="This is a required field" value={this.state.selectedValue2} onChange={this.onValue2Change} />;
                }
                break;
            case 4:
                if (operatorType == 13) {
    ...
    onOperatorChange(event: any) {
        ...
        if (selectedAttribute.length > 0 && (selectedAttribute[0].AttributeType === 6 || selectedAttribute[0].AttributeType === 7 || selectedAttribute[0].AttributeType === 5)) {
            let postValue = {
                ...
                queryOperator: selectedOperator[0].OperatorName
            }
            Remote.postData("/ResourceAttribute/GetOptions", postValue, (data: any) => {
                ...
                sender.setState({ operatorValue: selectedOperatorValue, selectedAttributeType: attributeType, selectedOperatorType: operatorType, attributeOptions: data, attributeAllOptions: data, selectedValue: '', selectedValue2: '' });
            }, (error: any) => {
                window.Dialog.alert(error)
            });
        }
    ...
    onValueChange(event: any) {
        ...
        if (event.target.value == "") {
            ...
            if (newCondition.Attribute.AttributeType === 3) {
                newCondition.Value = numberWithCommas(event.target.value);
    ...
    onValue2Change(event: any) {
        ...
        if (event.target.value == "") {
            ...
            if (newCondition.Attribute.AttributeType === 3) {
                newCondition.Value = this.state.selectedValue;
                newCondition.Value2 = numberWithCommas(event.target.value);
            }
            else {
                newCondition.Value = this.state.selectedValue;
        ...
        this.setState({ selectedValue2: newCondition.Value2 }, this.forceUpdate);
        this.props.onConditionChange(this.props.rowIndex, newCondition);
